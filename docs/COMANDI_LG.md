...
| Comando | Tipo | Scopo |
|---|---|---|
| `lgdock` | script | **Il comando da usare.** Lancia il container LegoPST e apre un terminale bash dentro, creando dinamicamente l'utente dell'host nel container così che i file scritti restino tuoi. Opzioni: `-d`/`--demo` installa la demo (legocad e sked) nella home e parte con quella; `-s`/`--socat` usa un socket bridge per l'X11, necessario via SSH/MobaXterm; `-p`/`--pull` aggiorna l'immagine prima di partire; `-l`/`--slim` usa l'**immagine snella** (`aguagliardi/legopst_slim:2.0`, stesso ambiente in meno della metà dello spazio) invece della completa; `-h` e `-v` per aiuto e versione. Si combinano: `lgdock -d -s`, `lgdock -l -d`. Attenzione: `-s` minuscola e' socat, `-l` e' l'immagine leggera. |
| `lgdock_multi` | script | Variante **multipiattaforma**: stessa interfaccia (`-h`, `-v`, `-d`, `-s`, `-l`) ma senza `--pull`, e con il rilevamento dei permessi — se `/var/run/docker.sock` esiste ma non è scrivibile dall'utente, ricade su `sudo docker` (chiedendo subito la password, invece di fallire a metà). Da usare dove `lgdock` non parte per questioni di permessi. |
...
| `lgmkstaz` | script | **Builder grafico delle pagine di faceplate**: apre un `r01.dat` (quello della directory corrente se non si passa nulla, come `compstaz`) e lo costruisce a video — griglia con l'origine in basso a sinistra, libreria dei 54 tipi da cui piazzare col mouse, selezione a rettangolo di più stazioni, trascinamento, copia/incolla, annulla e ripeti, pannello delle proprietà generato dalla grammatica degli oggetti e collegato alle variabili vere del modello (`variabili.edf`). Le stazioni si vedono con le **immagini reali** di `xstaz`, con sopra le scritte di quella stazione. Da `Verifica → Compila e verifica...` lancia `compstaz` per davvero su una copia scratch, con una chiave SHM isolata, e da lì apre l'**anteprima nel vero `xstaz`**. Si raggiunge anche da `lghmi`, *Tools → lgmkstaz*. Guida: [LGMKSTAZ.md](../Alg_legopc/LGMKSTAZ.md). |
