...
#  Description:		
#  %created_by:		lomgr %
#  %date_created:	Fri Aug 29 13:47:22 2003 %
if [ $# -eq 1 ]
then
cd $1
fi
...
net_sked 1 -num_snap $SNAP_S -num_bktk $BACK_T -num_camp_cr $CAMPIO -num_var_cr $NUM_VA -num_pert_active $PERTUR -num_spare_forsnap $SPARE_ -clear_pert_bktk $PERTCL -host_scada_name $SCAD_H -no_priolo $NO_PRI -time_bktk $TIME_BACK_T&
#

sleep 2

banco SUPERUSER  -num_snap $SNAP_S -num_bktk $BACK_T -num_camp_cr $CAMPIO -num_var_cr $NUM_VA -num_pert_active $PERTUR -num_spare_forsnap $SPARE_ -clear_pert_bktk $PERTCL $TIME_BACK_T -host_scada_name&

#ksh `which alg_respawn` banco SUPERUSER  -num_snap $SNAP_S -num_bktk $BACK_T -num_camp_cr $CAMPIO -num_var_cr $NUM_VA -num_pert_active $PERTUR -num_spare_forsnap $SPARE_ -clear_pert_bktk $PERTCL $TIME_BACK_T -host_scada_name&
fi
fi
fi
