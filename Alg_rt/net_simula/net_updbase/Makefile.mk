...
#       %created_by:    %
#       %date_created:  %
CFLAGS = -I$(LEGORT_INCLUDE) -D$(OS) $(C_FLAGS) $(VERSIONE) -I$(LEGOROOT_LIB) \
		-I$(LEGOROOT_INCLUDE)

LIBUTIL = $(LEGOROOT_LIB)/libRt.a $(LEGORT_LIB)/libsim.a \
	$(LEGORT_LIB)/libnet.a  $(LEGORT_LIB)/libipc.a \
	$(LEGOROOT_LIB)/libutil.a  $(LEGOROOT_LIB)/libRt.a


SORGENTI = net_updbase.c
OGGETTI  = net_updbase.o

all:  $(LEGORT_BIN)/net_updbase

makefile:Makefile
	make inc

$(LEGORT_BIN)/net_updbase: $(OGGETTI) $(LIBUTIL)
	cc  -o $(LEGORT_BIN)/net_updbase $(LINKER_OPTIONS) $(OGGETTI) \
        $(LIBUTIL) $(OTHER_LIB)
