...
*/
static char SccsID[] = "@(#)am3_r.c	1.4\t3/23/95";
/*
   modulo am3_r.c
   tipo 
   release 1.4
   data 3/23/95
   reserved @(#)am3_r.c	1.4
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
*/
int am3_l;       /* larghezza della stazione */
int am3_draw_h;  /* altezza dell'area di visualizzazione strumento
                    analogico  */
...
static void am3_draw1_callback(); /* callback della area di disegno  */
static void am3_del_callback();
static void am3_draw2_callback(); /* callback della area di disegno testi */
static void am3_btn2_callback();  /* callback del bottone centrale */
static void am3_btn1_callback();  /* callback del bottone sinistro */
static void am3_btn3_callback();  /* callback del bottone destro */
static void am3_init_text_callback();  /* callback del bottone destro */
static void am3_fine_text_callback();  /* callback del bottone destro */
static void am3_refresh();        /* callback da chiamarsi scaduto il
                                      timeout di refresh */
static void am3_edit_text_callback();
...
/*
 ...
 di fondo scala
 */
XPoint am3_posiz[]={{10,90},  /* minimo del valore */
...
for(i=0;i<4;i++)
	...
	sprintf(app,"%4d",val);
	XDrawString(display,sfondo,gc[0],am3_posiz[i].x,am3_posiz[i].y,
            app,strlen(app));
	}
...
static void am3_del_callback(w,info,str)
Widget w;
caddr_t info;
...
static void am3_edit_text_callback(w,info,str)
Widget w;
caddr_t info;
XmToggleButtonCallbackStruct *str;
{

if (str->set)  am3_init_text_callback(w,info,str); /* set bottone */
else           am3_fine_text_callback(w,info,str); /* reset */
}



static void am3_init_text_callback(w,info,str)
Widget w;
caddr_t info;
...
static void am3_fine_text_callback(w,info,str)
Widget w;
caddr_t info;
XmAnyCallbackStruct *str;
{
...
static void am3_draw1_callback(w,info,str)
Widget w;
caddr_t info;
XmDrawingAreaCallbackStruct *str;
{
...
static void am3_draw2_callback(w,info,str)
Widget w;
caddr_t info;
XmDrawingAreaCallbackStruct *str;
{
...
static void am3_btn2_callback(w,info,toggle)
Widget w;
caddr_t info;
...
static void am3_btn1_callback(w,info,toggle)
Widget w;
caddr_t info;
XmToggleButtonCallbackStruct *toggle;
{
 ...
 */
static void am3_btn3_callback(w,info,toggle)
Widget w;
caddr_t info;
XmToggleButtonCallbackStruct *toggle;
{
...
*/
        ...
        am3_set_button(stazione[is].incorso,WAIT,is);
        }
else
        {
        stazione[is].timer=0;
 ...
 */
                am3_set_button(slstaz,FISSO,is);
                }
        else
                {
                am3_set_button(!slstaz,WAIT,is);
                }
        }
