...
*/
static char SccsID[] = "@(#)gdisplayscal.c	1.5\t3/23/95";
/*
   modulo gdisplayscal.c
   tipo 
   release 1.5
   data 3/23/95
   reserved @(#)gdisplayscal.c	1.5
*/
/*
	contiene le routine di gestione degli oggetti di tipo DISPLAY_SCALATO
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
void displaysc_refresh();
void displaysc_del_callback();

float estr_sh();

void crdisplaysc(istaz,wbox,p_ogg,p_r02)
int	istaz;		/* indice stazione */
Widget	wbox;		/* indice widget padre */
OGGETTO	*p_ogg;		/* puntatore oggetto in tabella new_staz */
TIPO_DISPLAY_SCALATO *p_r02;	/* puntatore oggetto in tabella stazioni */
{
int i;
Widget wdisplay;
DATI_REFRESH *p_refr;
...
*/
XtAddCallback(wdisplay,XmNdestroyCallback,displaysc_del_callback,p_refr);

/* 
	aggiunge la callbacks  di refresh del display
*/

	if(add_refresh((XtCallbackProc)displaysc_refresh,(caddr_t)p_refr)==-1)
	{
		libera_trefr(p_refr);
...
void displaysc_del_callback(w,info,str)
Widget w;
caddr_t info;
...
p_r02= (TIPO_DISPLAY_SCALATO *) p_refr->pdati;
