...
/* converto la stringa in XtTranslation */
       ...
       if (*policy == '#')
       {
           ...
           {
               case 'r':
                 set_something_val(wid, XtNtranslations, (XtArgVal) tr);
               break;
               case 'a':
...
*/
      ...
      get_something(new_wid,XlNy0, (XtPointer)&y0);  
	...
	se il padre e' un composite controllo le eventuali rotazioni
      */
      ...
      {
      get_something_val(parent,XlNrotate, (XtPointer)&parent_rotate);  
      get_something_val(parent,XlNassRotate, (XtPointer)&parent_ass_rotate);  
      get_something_val(new_wid,XlNrotate, (XtPointer)&app_rotate);  
      get_something_val(new_wid,XlNassRotate, (XtPointer)&app_ass_rotate);  
	...
	if(((parent_rotate == ROTATE_90) || (parent_rotate == ROTATE_270))&&
        ...
        set_something_val(new_wid,XlNrotate,(XtArgVal)app_rotate);  
        sprintf(appo,"%d",app_rotate);
        XlSetResourceByName(&pagina->db,new_name,XlNrotate,appo);
        set_something_val(new_wid,XlNassRotate,(XtArgVal)app_ass_rotate);  
...
void istanzia_widget(Widget wid,XEvent *eve,String *param,Cardinal *nparam)
{
   ...
   classe = XtClass(wid); 
   get_something_val(wid,XmNwidth, (XtPointer) &w);
   get_something_val(wid,XmNheight, (XtPointer)&h);
...
// GUAG2025 forse False
         ...
         set_something_val(wgtsel,XlNselected,(XtArgVal)True);
      }
   }
}
...
for(i=0;i<num_figli;i++)
  ...
  for(j=0;j<num_nipoti;j++)
	...
	if((pag->dispreg_visual_mode != dispReg) && XlIsDispReg(m_nipoti[j]))
	    set_something_val(m_nipoti[j],XlNdispRegMode,(XtArgVal)dispReg);
	if((pag->port_visual_mode != port) && XlIsPort(m_nipoti[j]) && 
		!XlIsInterfacePort(m_nipoti[j]))
	    	set_something_val(m_nipoti[j],XlNvisibleMode,(XtArgVal)port);
	if((pag->interfaceport_visual_mode != interfacePort) &&  
		XlIsInterfacePort(m_nipoti[j]))
	    	set_something_val(m_nipoti[j],XlNvisibleMode,(XtArgVal)interfacePort);
	}
  XtFree(m_nipoti);
  }
XtFree(m_figli);
...
if(XlIsDispReg(w))
    set_something_val(w,XlNdispRegMode,(XtArgVal)dispReg);
if(XlIsPort(w) && !XlIsInterfacePort(w))
    	set_something_val(w,XlNvisibleMode,(XtArgVal)port);
if(XlIsInterfacePort(w))
    	set_something_val(w,XlNvisibleMode,(XtArgVal)interfacePort);
if(XlIsIconReg(w))
  {
  ...
  for(i=0;i<num_figli;i++)
	...
	if(XlIsDispReg(figli[i]))
    		set_something_val(figli[i],XlNdispRegMode,(XtArgVal)dispReg);
	if(XlIsPort(figli[i]) && !XlIsInterfacePort(figli[i]))
    		set_something_val(figli[i],XlNvisibleMode,(XtArgVal)port);
	if(XlIsInterfacePort(figli[i]))
    		set_something_val(figli[i],XlNvisibleMode,(XtArgVal)interfacePort);
	}
  }
}
