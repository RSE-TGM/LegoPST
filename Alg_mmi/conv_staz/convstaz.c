...
*/
static char SccsID[] = "@(#)convstaz.c	1.3\t10/20/93";
/*
   modulo convstaz.c
   tipo 
   release 1.3
   data 10/20/93
   reserved @(#)convstaz.c	1.3
*/
/**********************************************************
	programma convstaz
Converte il file descrittore delle stazioni r01.dat e produce
generando un numero appropriato di pagina tipo stazioni
per LEGOMMI
*********************************************************/
...
/*
	variabili globali per convrsione da stazione a LEGOMMI
*/
FILE *fp_staz[MAX_PAG];
int numero_el[MAX_PAG];
int last_num[MAX_PAG];
int max_x[MAX_PAG];
int max_y[MAX_PAG];
char nomi_ogg_pag[MAX_PAG][MAX_LUN_RIGA_ELENCO_WID];
...
printf("*******************************\n");
printf("lettura pagina %s - %s\n",pag.p[numero_pag].nome,
		pag.p[numero_pag].descrizione);
printf("indice pagina %d\n",ipag);

ApriFileMMI(ipag/*numero_pag*/,pag.p[numero_pag].nome,
		pag.p[numero_pag].descrizione);
  numero_pag++;
  return(0);
...
/* Lorenzo
 distruggi_var(id_sh);
*/
 exit (puts("\n Il programma CONVSTAZ termina per errore"));
 }
...
/*
	compila l' array degli indici stazioni relativi alle pagine
*/
